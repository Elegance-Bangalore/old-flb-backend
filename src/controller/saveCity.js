...
exports.saveCitiesData = async (req, res) => {
  try {
    const city = [
      {
        name: "Abhaneri",
    ...
    const cities = await City.insertMany(city);
    return res.status(201).send({ cities, message: "City added successfully" });
  } catch (error) {
    console.log(error);
...
exports.saveSpecifiCity = async (req, res) => {
  try{
    const {name, countryCode, stateCode, latitude, longitude} = req.body
    const city = new City({
      name,
      countryCode,
      stateCode,
    ...
    return res.status(201).send({cityData, message : "City added successfully"})
  }catch(error){
    console.log(error)
    return res.status(500).send({error : "Something Broke!"})
  }
